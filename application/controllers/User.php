...
class User extends CI_Controller {
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('passconf', 'Password','required|matches[password]', array('matches' => '%s tidak sesuai dengan password')
			);
		...
		} ini sudah dipakai, silakan ganti');
	...
	public function edit($id)
	{
		...
		$this->form_validation->set_message('min_length','{field} minimal 5 karakter');
		$this->form_validation->set_message('is_unique','{field} ini sudah di pakai','silakan ganti');
		$this->form_validation->set_error_delimiters('<span class="help-block">','</span>');
	...
	function username_check() {
		...
		if ($query->num_rows() >0) {
			$this->form_validation->set_message('username_check','{field} ini sudah dipake, silakan ganti');
			return FALSE;
		} else {
			return TRUE;
		}
